...
| 6 | **Order cross-device events by `server_time`**, not `effective_time`. Keep `effective_time` for display and late-arrival detection. Intra-device ordering stays `client_seq`. | The current key puts the untrusted value first. One phone four minutes off silently reorders an entire org's history on every projection rebuild — reproducibly, which makes it look intentional. |
