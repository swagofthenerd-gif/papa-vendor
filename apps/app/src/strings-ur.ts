...
export const STR_UR: StrTable = {
  ...
  networkLendDone: (label: string): string => `${label} Today board par hai — saman us par scan out karein.`,
  ...
  networkCrewRefusal: (reason: string): string =>
    ({
      no_job: 'Yeh job is phone par nahi.',
      cancelled: 'Cancel job par koi nahi jata.',
      no_user: 'Yeh banda roster par nahi.',
      bad_role: 'Crew role attendant ya driver hota hai.',
    }[reason] ?? reason),
